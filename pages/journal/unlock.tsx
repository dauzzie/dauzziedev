...
export default function JournalUnlock() {
  ...
  const nextRoute = typeof router.query.next === 'string' ? router.query.next : '/journal'
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch('/api/journal-gate', {
      method: 'POST',
    ...
    if (!response.ok) {
      setError('Wrong journal password. Try again.')
      setLoading(false)
      return
    }
  ...
  return (
    <>
      <PageSEO
        title={`Journal Access - ${siteMetadata.author}`}
        description="Password protected journal section"
      />
      <main className="mx-auto flex min-h-screen max-w-3xl items-center px-4 sm:px-8">
        <section className="apple-glass-card w-full p-8 md:p-12">
          ...
          <form className="mt-8 space-y-4" onSubmit={onSubmit}>
            <label htmlFor="journal-password" className="sr-only">
              Journal password
            </label>
            <input
              id="journal-password"
            ...
            >
              {loading ? 'Checking...' : 'Unlock journal'}
            </button>
